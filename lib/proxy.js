...
var Proxy = (function (_EventEmitter) {
  ...
  function Proxy() {
    ...
    if (opts.certAuthority) {
      ...
      (function () {
        ...
        wssServer = new _ws2['default'].Server({ server: _this._tlsSpoofingServer });

        wssServer.on('connection', function (ws, req) {
          ws.upgradeReq = req;
          _this.onWebSocketServerConnect(true, ws, req);
        });
      })();
    }
  }
  ...
  _createClass(Proxy, [{
    key: 'onWebSocketServerConnect',
    value: function onWebSocketServerConnect(isSSL, ws, upgradeReq) {
      var self = this;
      var ctx = {
        isSSL: isSSL,
      ...
      function makeProxyToServerWebSocket() {
        ...
        ctx.proxyToServerWebSocket = new _ws2['default'](ctx.proxyToServerWebSocketOptions.url, ctx.proxyToServerWebSocketOptions);
    ...
    key: 'onWebSocketFrame',
    value: function onWebSocketFrame(ctx, type, fromServer, data, flags) {
      var self = this;
      var destWebSocket = fromServer ? ctx.clientToProxyWebSocket : ctx.proxyToServerWebSocket;
      if (destWebSocket.readyState === _ws2['default'].OPEN) {
        switch (type) {
          case 'message':
    ...
    value: function onWebSocketError(ctx, err) {
      if (ctx.proxyToServerWebSocket && ctx.clientToProxyWebSocket.readyState !== ctx.proxyToServerWebSocket.readyState) {
        if (ctx.clientToProxyWebSocket.readyState === _ws2['default'].CLOSED && ctx.proxyToServerWebSocket.readyState === _ws2['default'].OPEN) {
          ctx.proxyToServerWebSocket.close();
        } else if (ctx.proxyToServerWebSocket.readyState === _ws2['default'].CLOSED && ctx.clientToProxyWebSocket.readyState === _ws2['default'].OPEN) {
          ctx.clientToProxyWebSocket.close();
        }
      }
    }
  }, {
    key: 'parseHostAndPort',
    value: function parseHostAndPort(req, defaultPort) {
    ...
    key: 'parseHost',
    value: function parseHost(hostString, defaultPort) {
